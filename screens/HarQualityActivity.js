...
export default class HarQualityActivity extends React.Component {
  ...
  componentDidMount() {
    ...
    try {
      AsyncStorage.getItem('houseNumber')
        .then((house) => {
  ...
  resetTrussCuttingRadioButton = () => {
    ...
    this.setState(
      ...
      () => {
        this.setState({
          trussCuttingOption4: this.state.radiorussCuttingItems4[0].label,
        });
      },
    );
    //END
  ...
  calculateTrussCuttingQualityPercentage = () => {
    setTimeout(() => {
      if (this.state.trussCuttingOption1 === 'Pass') {
        count = count + 25;
      } else {
      }

      if (this.state.trussCuttingOption2 === 'Pass') {
        count = count + 25;
      } else {
      }

      if (this.state.trussCuttingOption3 === 'Pass') {
        count = count + 25;
      } else {
      }

      if (this.state.trussCuttingOption4 === 'Pass') {
        count = count + 25;
      } else {
      }
  ...
  saveTrussCuttingDataToDB = () => {
    ...
    const {workersName} = this.state;
    const {jobSelected} = this.state;
    const {trussCuttingOption1} = this.state;
    const {trussCuttingOption2} = this.state;
    const {trussCuttingOption3} = this.state;
    const {trussCuttingOption4} = this.state;
    const {data_send} = this.state;
    ...
    if (auditorsName) {
      if (houseNumber) {
        if (rowNumber) {
          if (workersName) {
            if (jobSelected) {
              if (trussCuttingOption1) {
                if (trussCuttingOption2) {
                  if (trussCuttingOption3) {
                    if (trussCuttingOption4) {
                      if (this.state.isItConnected === 'Online') {
                        console.log('Connected to internet');
                        ...
                        const url = `${scriptUrl}?
                          ...
                          that.state.adiCode
                        }&data1=${that.state.trussCuttingOption1}&data2=${
                          that.state.trussCuttingOption2
                        }&data3=${that.state.trussCuttingOption3}&data4=${
                        ...
                        realm.write(() => {
                          ...
                          realm.create('qualitySheet', {
                            ...
                            comments: that.state.comments,
                            quality_percent: that.state.qualityPercentage,
                            data1: that.state.trussCuttingOption1,
                            data2: that.state.trussCuttingOption2,
                            data3: that.state.trussCuttingOption3,
                            data4: that.state.trussCuttingOption4,
                            data5: '',
                            data6: '',
                            data7: '',
                            data8: '',
                          ...
                          this.resetTrussCuttingRadioButton();
                          AsyncStorage.clear();
                          AsyncStorage.removeItem('auditorsName');
                          AsyncStorage.removeItem('workersName');
                          AsyncStorage.removeItem('houseNumber');
                      ...
                      this.setState({isLoading: false});
                      alert(
                        'Choose one option from truss cutting quality check 4',
                      );
                    }
                  } else {
                    this.setState({isLoading: false});
                    alert(
                      'Choose one option from truss cutting quality check 3',
                    );
                  }
                } else {
                  this.setState({isLoading: false});
                  alert('Choose one option from truss cutting quality check 2');
                }
              } else {
                this.setState({isLoading: false});
                alert('Choose one option from truss cutting quality check 1');
              }
            } else {
              this.setState({isLoading: false});
              alert('Please select job');
  ...
  navigateToScreen = () => {
    this.props.navigation.navigate('NameJobSelector', {
        ...
        <ScrollView
          ...
          <View
            ...
            <DropDownPicker
              ...
              dropDownStyle={{backgroundColor: '#fafafa'}}
              onChangeItem={(item) =>
                this.updateTextInput(item.value, 'houseNumber')
              }
          ...
          {this.state.jobSelected === 'Clip And Prune' ? (
            <View>
              ...
              <Text style={styles.titleHeading2Text}>
                5) Accuracy: prune to the correct number and remove flag leaf +
                King fruit <Text style={styles.redColor}>(3 each)</Text>{' '}
              </Text>
          ...
          {this.state.jobSelected === 'Truss Cutting' ? (
            <View>
              ...
              <TouchableOpacity
                style={styles.buttonContainer}
                onPress={this.calculateTrussCuttingQualityPercentage}>
                <Text style={styles.buttonText1}>Submit</Text>
              </TouchableOpacity>
            </View>
          ) : null}
